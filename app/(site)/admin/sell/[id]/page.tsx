...
import { getSellOrderById } from "@/lib/db/queries"
...
export default async function SellOrderDetailsPage({
  params,
}: {
  params: { id: string }
}) {
  const order = await getSellOrderById(params.id)
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-2xl font-bold">جزئیات سفارش فروش</h1>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                <p>شهر: {order.city || "نامشخص"}</p>
                <p>طول جغرافیایی: {order.lng || "نامشخص"}</p>
                <p>عرض جغرافیایی: {order.lat || "نامشخص"}</p>
              </div>
              {order.city && order.lng && order.lat && (
